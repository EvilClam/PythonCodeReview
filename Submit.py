...
class UtilFunctions:
    ...
    def initRepositories(self, repositoryIDs):
        ...
        '''
        repositories = {}
        isDir = path.isdir(absPath + "/Review")
        if (not isDir):
            os.mkdir(absPath + "/Review")
        
        for currentID in repositoryIDs.keys():
            currentPath = absPath + "/Review/" + currentID
            isDir = path.isdir(currentPath)
            if (not isDir):
                os.mkdir(currentPath)
                repositories[currentID] = pygit2.clone_repository(repositoryIDs[currentID].getUrl(),currentPath())
